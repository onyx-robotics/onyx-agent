...
const PROFILE_USAGE =
  "Usage: onyx profile list | onyx profile use <name> | onyx profile worker get [profile] | onyx profile worker set [profile] --agent codex|claude|opencode [--model <model>] | onyx profile worker clear [profile] (--all | --agent | --model codex|claude|opencode)"
...
export async function commandProfileList() {
  ...
  for (const [name, profile] of entries) {
    ...
    console.log(
      `${marker} ${name}\t${profile.teamName}\t${profile.teamId}\t${profile.apiUrl}\tCLI session (${profile.credentialStore})\t${workerListSummary(profile)}`
    )
  }
}
