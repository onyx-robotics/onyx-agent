...
function createSyncSupervisor({
  ...
  const runFlushJob = async (job: SyncQueueFlushJob) => {
    await flushOutbox(root, args, { quiet: true })
      .then(async (result) => {
...
function createPresenceSupervisor({
  ...
  const snapshot = async () => {
    ...
    const pendingSyncCount = await pendingResearchSyncCount(root).catch(() => 0)
    ...
    const response = await syncResearchPresence(
      {
        ...
        site: {
          pendingSyncCount,
          pushQueueDepth: syncQueueDepth?.() ?? 0,
          activeWorkerCount,
          providerBackoff:
...
async function computeSessionFinalizationStatus({
  ...
  for (const manifest of manifests) {
    ...
    const hasUnresolvedSalvage =
      finalization.finalizationStatus === "salvaged_unmeasured" ||
    ...
    if (finalization.rootDriftStatus === "dirty") {
      incompleteReasons.push(`worker ${manifest.workerId} detected root drift`)
    }
  }
...
async function cachedResearchSessionLive({
  sessionId,
  args,
  nowMs,
  liveCache,
  ...
  args: Args
  nowMs: number
  liveCache?: ResearchStopLiveCache
  liveTtlMs: number
}) {
  if (
    liveCache &&
    liveCache.live &&
    nowMs - liveCache.fetchedAtMs < liveTtlMs
  ) {
    return liveCache.live
  }
  const live = await getResearchSessionLive(sessionId, args)
  if (liveCache) {
...
export async function collectResearchStopReasons({
  ...
  const workerResearchDeadline = process.env.ONYX_RESEARCH_DEADLINE_AT
    ...
    : Number.NaN
  if (Number.isFinite(workerResearchDeadline) && nowMs >= workerResearchDeadline) {
  ...
  try {
    ...
    if (remote.session.maxExperiments !== null) {
      try {
        const live = await cachedResearchSessionLive({
          sessionId,
          args,
          nowMs,
          liveCache,
        ...
        if (live.budget.expiredReservationCount > 0) {
          addStopReason(
            "reservation_expired",
            `${live.budget.expiredReservationCount} experiment reservation(s) expired`
          )
        }
      } catch {
        // State checks still provide the normal budget stop signal.
      }
    }
    if (
      ...
      addStopReason(
        "session_terminal",
        `remote session ${remote.session.status}`
      )
    }
  } catch {
...
export function createResearchSessionStopChecker({
  ...
  const liveCache: ResearchStopLiveCache = { fetchedAtMs: 0, live: null }
  return {
    check({
      iteration = null,
      nowMs = Date.now(),
...
export function providerBackoffReasonForResult(
  ...
  if (/rate limit|too many requests|429|quota|throttl|retry[- ]?after/i.test(message)) {
    return "rate_limit"
  }
  if (/overloaded|capacity|temporarily unavailable|503|529|server busy/i.test(message)) {
    return "overloaded"
  }
  if (
...
export async function commandResearchRun(args: Args) {
  ...
  const persistProviderBackoff = async (
    providerBackoff: {
      reason: string
      until: string
      attempt?: number
          ...
          .then(async (result) => {
            ...
            if (result.status === "completed") {
              ...
              if (providerBackoffAttempt > 0 || providerBackoffUntil > 0) {
                ...
                await persistProviderBackoff(null)
              }
  ...
  console.log(
    `Research run ${finalStatus}: launched=${launched}${
      maxLaunches ? `/${maxLaunches}` : ""
    } completed=${completed} failed=${failed} stopped=${stopped}; finalization=${reportedFinalizationStatus}; ${finalSync.pending} sync record(s) pending.`
  )
}
