...
- Setup: ${input.setupId} v${input.setupVersion}
...
${optionalContextLine("Peer lane state", input.sessionStatePath)}- Tool API: ${input.toolApiPath}
...
1. Read the research spec, campaign brief, peer state, recent experiments, git status, and relevant source files. Understand the workload before editing.
...
7. Periodically update a concise lane summary with \`onyx summary upsert\` if available; otherwise include the summary in final output.
8. Run \`onyx sync\` or \`onyx push\` periodically when network access is available.
...
- Use loop statuses from \`onyx exp run\`: \`succeeded\`, \`failed\`, and \`checks_failed\`. Do not mark autonomous attempts \`accepted\` or \`rejected\`; those are for human curation.
...
- Do not delete campaigns or experiments. Deletion/tombstones are human/controller actions.
- Use \`onyx exp list --limit 20\` for recent history and \`onyx exp list --grep <regex>\` before repeating an idea.
- Use \`onyx summary upsert\` for promising ideas, dead-end themes, and transfer notes. If you keep a local backlog file, avoid protected setup paths and commit it normally.
