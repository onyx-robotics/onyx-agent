...
- Campaign brief command for deeper context: \`"$ONYX_WORKER_BIN" research brief --campaign "$ONYX_CAMPAIGN_NAME" --session "$ONYX_SESSION_ID" --hypothesis "$ONYX_HYPOTHESIS_ID"\`
...
The supervisor launched this worker with \`"$ONYX_WORKER_BIN"\`, \`ONYX_WORKER_CONTEXT\`, and an isolated \`ONYX_HOME\`. Use \`"$ONYX_WORKER_BIN"\` for all Onyx commands; the full \`onyx\` CLI is the user/orchestrator surface and is not part of the worker runtime. If an Onyx command reports an auth or context problem, stop and summarize the exact error instead of changing profiles or config.
...
3. Review the latest research state
  - Use \`"$ONYX_WORKER_BIN" research session-state-brief --json\` as your single routine context source. It includes complete assigned-hypothesis guidance, peer summaries, accepted results, and bounded full knowledge items. Run \`"$ONYX_WORKER_BIN" research brief --campaign "$ONYX_CAMPAIGN_NAME" --session "$ONYX_SESSION_ID" --hypothesis "$ONYX_HYPOTHESIS_ID"\` only when you need a fuller prose brief.
...
5. Edit only in-scope project files to implement the experiment idea, make exactly one clean commit, then resume the same workflow with \`"$ONYX_WORKER_BIN" exp run --resume --auto\`. If blocked, inspect \`"$ONYX_WORKER_BIN" workflow status --blocked\`; use \`"$ONYX_WORKER_BIN" tools run <tool-id>\` only for diagnostics.
