...
3. Review the latest research state
  - Use \`onyx-worker research session-state-brief --json\` as your single routine context source. It includes complete assigned-hypothesis guidance, peer summaries, accepted results, and bounded full knowledge items. Run \`onyx-worker research brief --campaign "$ONYX_CAMPAIGN_NAME" --session "$ONYX_SESSION_ID" --hypothesis "$ONYX_HYPOTHESIS_ID"\` only when you need a fuller prose brief.
