...
8. Start the session with `onyx research start --campaign <slug> --workers <n> --agent codex|claude --hypotheses <plans.json>`. Use `onyx research hypotheses --example > hypotheses.json` as a setup-aware template when drafting initial hypotheses. Launch workers with the printed `onyx worker run --session ... --hypothesis ...` commands, using `--agent codex`, `--agent claude`, or `--worker-command` when appropriate.
