...
8. Start the session with inline JSON: `onyx research start --campaign <slug> --workers <n> --agent codex|claude --hypotheses '<json-array>'`. Treat `--max-iterations` as a maximum cap, not a target count. Use `onyx research hypotheses --example` as a setup-aware template when drafting initial hypotheses, then pass the JSON array inline. Launch workers with the printed `onyx worker run --session ... --hypothesis ...` commands, using `--agent codex`, `--agent claude`, or `--worker-command` when appropriate.
