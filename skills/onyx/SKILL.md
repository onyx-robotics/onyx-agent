...
1. Ask or infer the goal, immutable metric name/unit/direction, editable and protected scopes, evaluator, reset/readiness/safety tools, resource limits, hypotheses, worker capacity, and experiment/deadline bound.
2. **PAUSE FOR USER APPROVAL** - You must pause here and ask the user if the items from setup step number one are correct. Work collaboratively with the user to correctly identify each of the items from step one.
...
5. **PAUSE FOR USER APPROVAL** - You must pause here and ask the user for approval of the setup. The setup process is critical to the success of the rest of the research process, so work collaboratively with them to discuss any notable details. You should point out any concerns you have or things the user should be aware of about the setup and research being worked on, so that you and the user collaboratively come up with the best setup possible.
...
1. Ask the user for hypotheses to explore or seed them if the user asks you to. Hypotheses are durable conceptual options, not branches or mutable git bases. Adding or reopening affects future sessions only. Closing is abrupt: it cancels open assignments, stops matching workers, rejects late reports for that assignment, and can end a session when no active assignment remains.
...
2. Start research with `onyx research run --campaign <slug> [--workers <n>] [--hypothesis <name-or-id>]... [--base <ref>] [--hypothesis-base <hypothesis>=<ref|experiment:id>]... (--experiments <n> | --max-minutes <n>)`. There is more information on the research workers below.
...
3. Monitor the research process, handle user inputs, and keep the research moving along successfully. Communicate to the user the research progress and anything the user should be alerted of.
...
Workers use `ONYX_PROJECT_ROOT` as their source-editing root and `"$ONYX_WORKER_BIN" research session-state-brief --json` for their single bounded routine context. `ONYX_WORKER_BIN` is the supervisor-verified absolute worker CLI; workers never resolve `onyx-worker` through `PATH`. The brief contains complete immutable assigned-hypothesis guidance, concise peer and accepted-result summaries, complete selected knowledge items, stop guidance, and progress; workers can use explicit list commands for deeper history. They must not ask the user questions, launch agents, use the full control-plane CLI, or mutate protected setup paths/runtime files.

For each attempt, a worker runs `"$ONYX_WORKER_BIN" exp run --campaign "$ONYX_CAMPAIGN_NAME" --auto`, makes exactly one scoped clean commit, resumes with `"$ONYX_WORKER_BIN" exp run --resume --auto`, and reports with `"$ONYX_WORKER_BIN" exp log`. A `recorded` response means delivery succeeded; the worker does not wait for acceptance. If the CLI's bounded reporting retry fails, the worker stops new work, leaves the pending attempt for teardown, and exits instead of retrying indefinitely. When no useful experiment remains, it records an explicit finish marker with `"$ONYX_WORKER_BIN" research finish --reason <reason> --summary <text>`. It publishes useful reusable knowledge when appropriate. Git holds immutable experiment commits/refs; Supabase owns session settlement, accepted indexes, experiment facts, and provenance. Best results and summary counts are computed from indexed experiment facts when read.
